...
import { Node } from '@/engine/pathfinding'
...
import { isValidTilePosition } from '@/utils/helpers'

import { AVATAR_DIMENSIONS } from '@/modules/avatar/constants'
...
export default class Pathfinder {
	...
	constructor(tileMap: TileMap, cubeMap: CubeMap) {
	...
	#getNeighborPositions(position: Point3D) {
		...
		return neighborPositions.filter(position =>
			isValidTilePosition(position, this.#tileMap),
		)
	}
	...
	#isObstacle(neighborNode: Node, currentNode: Node) {
		const tallestCubeAtNode = this.#cubeMap.findTallestCubeAt(
